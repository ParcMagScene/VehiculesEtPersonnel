...
export function setupPersonsRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/persons', authenticateToken, (req, res) => {
    try {
      ...
      for (const sk of allSkills) {
        ...
        skillsByPerson[sk.person_id].push({
  ...
  app.get('/api/persons/:id', authenticateToken, (req, res) => {
    try {
      ...
      person.availabilities = db
        .prepare('SELECT * FROM availabilities WHERE person_id = ? ORDER BY start_date')
        .all(person.id);

      person.assignments = db
        .prepare(
          `
  ...
  app.post(
    ...
    authenticateToken,
    requireAdmin,
    validate(personnelImportSchema),
    (req, res) => {
      try {
      ...
      `);
        ...
        const importAll = db.transaction(() => {
          for (const entry of analysis) {
            ...
            const mapped = TYPE_MAP[(entry.type_csv || '').toLowerCase()] || {
              type: 'permanent',
              contract_type: null,
            };
            ...
            if (entry.action === 'create') {
              insertStmt.run(
                ...
                entry.portable || null,
                mapped.type,
                mapped.contract_type || null,
                entry.code_libre || null,
                entry.cp || null,
                entry.ville || null,
                req.user.id,
                req.user.id,
        ...
        addToHistory(
          ...
          'bulk',
          'import_csv',
          { created, updated, skipped: importSkipped, total: data.length },
          req.user.id,
          req.user.name,
        );
...
export function setupSkillsRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.post('/api/skills', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      const result = db
        .prepare('INSERT INTO skills (name, category, description) VALUES (?, ?, ?)')
  ...
  app.put('/api/skills/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      if (!updated)
        return res.status(404).json({ success: false, error: 'Compétence non trouvée' });
  ...
  app.post('/api/positions', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      const result = db
        .prepare('INSERT INTO positions (name, category, is_common) VALUES (?, ?, ?)')
  ...
  app.get('/api/missions', authenticateToken, (req, res) => {
    try {
      ...
      for (const a of allAssignments) {
        ...
        assignmentsByMission[a.mission_id].push({
          ...a,
          skills: skillsByPerson[a.person_id] || [],
        });
      }
  ...
  app.post('/api/assignments', authenticateToken, (req, res) => {
    try {
      ...
      if (!a.mission_id || !a.person_id) {
        return res
          .status(400)
          .json({ success: false, error: 'mission_id et person_id sont requis' });
      }
      ...
      alertAssignmentCreated(
        ...
        req.user.name,
      ).catch((err) => logger.warn('Email alerte affectation échoué:', err.message));
      ...
      if (error.message.includes('UNIQUE constraint')) {
        return res
          .status(409)
          .json({ success: false, error: 'Cette personne est déjà affectée à cette mission' });
      }
      logger.error(error);
      res.status(500).json({ success: false, error: 'Erreur serveur interne' });
    }
  ...
  app.put('/api/assignments/:id', authenticateToken, (req, res) => {
    try {
      ...
      const existing = db
        ...
        .get(req.params.id);
      if (!existing)
        return res.status(404).json({ success: false, error: 'Affectation non trouvée' });

      // Si statut change vers confirmed/refused, enregistrer responded_at
      const respondedAt =
        ['confirmed', 'refused'].includes(a.status) && existing.status !== a.status
      ...
      if (a.status === 'confirmed') {
        ...
        if (mission && mission.status === 'open') {
          ...
          if (pending.c === 0) {
            db.prepare(
              "UPDATE missions SET status = 'staffed', modified_at = CURRENT_TIMESTAMP WHERE id = ?",
