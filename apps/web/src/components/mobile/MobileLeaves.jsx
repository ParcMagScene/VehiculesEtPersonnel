...
function MobileLeaves({ currentUser, onBack }) {
  ...
  const handleDecision = async (leaveId, decision, reason = '') => {
    try {
      ...
      alert('Erreur: ' + (e.message || 'Décision impossible'));
    }
  };
  ...
  const filteredLeaves = leaves
    ...
    .sort(
      (a, b) => new Date(b.created_at || b.start_date) - new Date(a.created_at || a.start_date),
    );
        ...
        <div className="ml-header-actions">
          {isAdmin && pendingCount > 0 && (
            <Button
              ...
              aria-label={`Validations en attente (${pendingCount})`}
            >
              <Clock size={16} />
      ...
      <div className="ml-content">
        ...
        <div className="ml-balances">
          ...
          <div className="ml-balances-grid">
            {balances.length > 0 ? (
              balances.map((b, i) => {
                const typeInfo = LEAVE_TYPE_LABELS[b.leave_type || b.leaveType] || {
                  label: b.leave_type || b.leaveType,
                  icon: '📋',
                  color: STATUS_COLORS.neutralSoft,
                };
                return (
                  <div
                    key={i}
                    className="ml-balance-chip"
...
function LeaveCard({ leave, onClick }) {
  const typeInfo = LEAVE_TYPE_LABELS[leave.leave_type || leave.leaveType] || {
    label: leave.leave_type || 'Congé',
    icon: '📋',
    color: STATUS_COLORS.neutralSoft,
  };
    ...
    >
      ...
      <div className="ml-leave-info">
        ...
        <div className="ml-leave-dates">
          ...
          {formatDate(leave.end_date || leave.endDate)}
          <span className="ml-leave-days">{days}j</span>
        </div>
      </div>
      <div
        className="ml-leave-status"
    ...
    <div className="ml-form">
        ...
        <div className="ml-type-grid">
          {Object.entries(LEAVE_TYPE_LABELS).map(([key, info]) => (
            <Button
              ...
              style={
                leaveType === key ? { borderColor: info.color, background: info.color + '15' } : {}
              }
        ...
        <div className="ml-half-day">
          {['full', 'morning', 'afternoon'].map((h) => (
            <Button
              variant="ghost"
              key={h}
              className={`ml-half-btn ${startHalf === h ? 'active' : ''}`}
              onClick={() => setStartHalf(h)}
            >
        ...
        <div className="ml-half-day">
          {['full', 'morning', 'afternoon'].map((h) => (
            <Button
              variant="ghost"
              key={h}
              className={`ml-half-btn ${endHalf === h ? 'active' : ''}`}
              onClick={() => setEndHalf(h)}
            >
              {h === 'full' ? 'Journée' : h === 'morning' ? 'Matin' : 'Après-midi'}
            </Button>
          ))}
      ...
      {(workingDays !== null || calculating) && (
        <div className="ml-working-days">
          ...
          {calculating
            ? 'Calcul...'
            : `${workingDays} jour${workingDays > 1 ? 's' : ''} ouvré${workingDays > 1 ? 's' : ''}`}
        </div>
      )}
...
function LeaveDetail({ leave, isAdmin, onDecision, onCancel }) {
  ...
  return (
    <div className="ml-detail">
      <div
        className="ml-detail-header"
        style={{ background: typeInfo.color + '15', borderLeftColor: typeInfo.color }}
      >
        <span className="ml-detail-icon">{typeInfo.icon}</span>
        <div>
          <div className="ml-detail-type">{typeInfo.label}</div>
          <div
            className="ml-detail-status"
            style={{ background: statusInfo.bg, color: statusInfo.color }}
          >
      ...
      <div className="ml-detail-body">
        <DetailRow
          ...
          label="Demandeur"
          value={leave.person_name || leave.personName || '—'}
        />
        <DetailRow
          className="ml-detail-row"
          label="Du"
          value={formatDate(leave.start_date || leave.startDate)}
        />
        <DetailRow
          className="ml-detail-row"
          label="Au"
          value={formatDate(leave.end_date || leave.endDate)}
        />
        ...
        {(leave.reason || leave.comment) && (
          <DetailRow
            className="ml-detail-row"
            label="Motif"
            value={leave.reason || leave.comment}
          />
        )}
          ...
          return (
            <div key={leave.id} className="ml-leave-card ml-admin-card">
              ...
              >
                ...
                <div className="ml-leave-info">
                  ...
                  <div className="ml-leave-dates">
                    {formatDate(leave.start_date || leave.startDate)} →{' '}
                    {formatDate(leave.end_date || leave.endDate)}
                    <span className="ml-leave-days">
                      {leave.working_days || leave.workingDays || '?'}j
                    </span>
                  </div>
                </div>
              </div>
              <div className="ml-admin-actions">
                <Button
                  variant="ghost"
                  className="ml-quick-accept"
                  onClick={() => onDecision(leave.id, 'accepted')}
                >
                  <CheckCircle size={18} /> Accepter
                </Button>
                <Button
                  variant="ghost"
                  className="ml-quick-reject"
                  onClick={() => onDecision(leave.id, 'refused')}
                >
