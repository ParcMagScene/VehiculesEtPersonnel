...
export function setupCatalogRoutes(app, authenticateToken, requireWriteAccess) {
  ...
  app.get('/api/catalog/equipment/:id', authenticateToken, (req, res) => {
    try {
      ...
      if (item.default_flightcase_id) {
        item.defaultFlightcase = db.prepare('SELECT * FROM flightcases WHERE id = ?').get(item.default_flightcase_id);
      }
  ...
  app.post('/api/catalog/equipment', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      addToHistory('equipment_catalog', id, 'create', { name, reference, family }, req.user?.id, req.user?.name);
  ...
  app.put('/api/catalog/equipment/:id', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      addToHistory('equipment_catalog', req.params.id, 'update', req.body, req.user?.id, req.user?.name);

      const updated = db.prepare('SELECT * FROM equipment_catalog WHERE id = ?').get(req.params.id);
      res.json(updated);
    } catch (error) {
      if (error.message.includes('UNIQUE constraint')) {
        return res.status(409).json({ error: 'Référence déjà existante' });
  ...
  app.delete('/api/catalog/equipment/:id', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      addToHistory('equipment_catalog', req.params.id, 'delete', { name: existing.name }, req.user?.id, req.user?.name);
...
export function setupFlightcasesRoutes(app, authenticateToken, requireWriteAccess) {
  ...
  app.post('/api/flightcases', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id, name, internal_code || null,
        dimensions ? JSON.stringify(dimensions) : null,
        capacity || 1, category || null, texture || null,
        metadata ? JSON.stringify(metadata) : null,
        now, now
      );

      addToHistory('flightcase', id, 'create', { name, internal_code }, req.user?.id, req.user?.name);
  ...
  app.delete('/api/flightcases/:id', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      db.prepare('UPDATE equipment_catalog SET default_flightcase_id = NULL WHERE default_flightcase_id = ?').run(req.params.id);

      db.prepare('DELETE FROM flightcases WHERE id = ?').run(req.params.id);
      addToHistory('flightcase', req.params.id, 'delete', { name: existing.name }, req.user?.id, req.user?.name);
...
export function setupTruckModelsRoutes(app, authenticateToken, requireWriteAccess) {
  ...
  app.post('/api/trucks/models', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        id, name, type || null, internal_code || null,
        dimensions ? JSON.stringify(dimensions) : null,
  ...
  app.put('/api/trucks/models/:id', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      addToHistory('truck_model', req.params.id, 'update', req.body, req.user?.id, req.user?.name);
...
export function setupReservationEquipmentRoutes(app, authenticateToken) {
  ...
  app.post('/api/reservations/:id/equipment', authenticateToken, (req, res) => {
    try {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(
        id, req.params.id, equipment_id,
        quantity || 1,
        effectiveFlightcaseId,
        metadata ? JSON.stringify(metadata) : null
      );

      addToHistory('reservation_equipment', id, 'assign', {
  ...
  app.delete('/api/reservations/:reservationId/equipment/:linkId', authenticateToken, (req, res) => {
    try {
      ...
      addToHistory('reservation_equipment', req.params.linkId, 'remove', {
  ...
  app.get('/api/reservations/:id/chargement-export', authenticateToken, (req, res) => {
    try {
      ...
      if (vehicle) {
        ...
        truckModel = db.prepare(
          "SELECT * FROM truck_models WHERE name LIKE ? OR internal_code LIKE ? LIMIT 1"
