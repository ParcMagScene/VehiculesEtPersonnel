...
function LocationIconsTab({ _currentUser, refreshKey, onPreviewChange }) {
  ...
  const handleRuleChange = (index, field, value) => {
    setRules((prev) => {
      ...
      if (onPreviewChange) {
        onPreviewChange({
          iconRules: next.map((r) => ({ keyword: r.keyword, gif_filename: r.gifFilename })),
        });
      }
      return next;
    });
    ...
    <div className="dtv-location-icons">
      ...
      <div className="dtv-section">
        <SectionHeader
          className="dtv-section-title"
          icon={<Film size={16} />}
          title={`Galerie des icônes (${gifs.length})`}
        />
        {gifs.length === 0 ? (
      ...
      <div className="dtv-section">
        <SectionHeader
          className="dtv-section-title"
          icon={<Film size={16} />}
          title="Associer des icônes aux types de tâches"
        />
        <p className="dtv-hint">
          Définissez quelle icône afficher sur l'écran TV selon le type de tâche.
        </p>

        <div className="dtv-rules-list">
          {rules.length === 0 && <div className="dtv-empty-hint">Aucune règle définie.</div>}
          {rules.map((rule, index) => (
            <div key={index} className="dtv-icon-rule">
              <div
                className="dtv-icon-selector"
                role="button"
                tabIndex={0}
                onClick={() => setShowMosaic(index)}
              >
                {rule.gifFilename ? (
