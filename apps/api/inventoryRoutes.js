...
export function setupInventoryRoutes(app, authenticate) {
  ...
  app.get('/api/inventory/locations', authenticate, (req, res) => {
    try {
      ...
      const params = [];
      if (depot) { sql += ' AND depot_number = ?'; params.push(depot); }
  ...
  app.post('/api/inventory/locations', authenticate, validate(locationSchema), (req, res) => {
    try {
      ...
      const result = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(name, code, depot_number || 1, type || 'storage', zone, floor, capacity, address, gps_lat, gps_lon, parent_id);
  ...
  app.put('/api/inventory/locations/:id', authenticate, validate(locationUpdateSchema), (req, res) => {
    try {
      ...
      `).run(
        name || existing.name, code || existing.code, depot_number ?? existing.depot_number,
        type || existing.type, zone ?? existing.zone, floor ?? existing.floor,
        capacity ?? existing.capacity, address ?? existing.address,
        gps_lat ?? existing.gps_lat, gps_lon ?? existing.gps_lon,
        parent_id ?? existing.parent_id, is_active ?? existing.is_active, req.params.id
      );
  ...
  app.post('/api/inventory/prices', authenticate, validate(priceSchema), (req, res) => {
    try {
      ...
      const result = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(stock_item_id, supplier_id, source || 'manual', price_ht, currency || 'EUR', quantity_break || 1, valid_from, valid_to, reference);
  ...
  app.get('/api/inventory/price-engine/:itemId', authenticate, (req, res) => {
    try {
      ...
      res.json({
        item: { id: item.id, name: item.name, reference: item.reference, current_price: item.unit_price },
        analysis,
      });
    } catch (err) {
    ...
    try {
      ...
      const insertMany = db.transaction((entries) => {
        for (const e of entries) {
          insert.run(stock_item_id, e.supplier_id || null, e.source || 'import', e.price_ht, e.reference || null);
        }
      });
      insertMany(prices);
      
      // Recalculer après insertion
      const allPrices = db.prepare('SELECT * FROM inventory_price_history WHERE stock_item_id = ? ORDER BY created_at DESC').all(stock_item_id);
      ...
      if (analysis.confidence >= 70 && analysis.recommended_price > 0) {
        db.prepare('UPDATE stock_items SET unit_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
  ...
  app.put('/api/inventory/anomalies/:id', authenticate, validate(anomalyUpdateSchema), (req, res) => {
    try {
      ...
      db.prepare(`UPDATE inventory_anomalies SET status = ?, resolved_by = ?, resolved_at = ? WHERE id = ?`)
  ...
  app.post('/api/inventory/anomalies/detect', authenticate, (req, res) => {
    try {
      ...
      for (const item of items) {
        const prices = db.prepare('SELECT price_ht FROM inventory_price_history WHERE stock_item_id = ?').all(item.id);
        ...
        if (item.unit_price < iqr.lowerFence || item.unit_price > iqr.upperFence) {
          const deviation = iqr.median > 0 ? ((item.unit_price - iqr.median) / iqr.median * 100) : 0;
          // Éviter les doublons
          ...
          if (!existing) {
            insertAnomaly.run(
              ...
              iqr.median, item.unit_price, Math.round(deviation * 100) / 100
            );
      ...
      for (const item of negatives) {
        ...
        `).get(item.id);
        if (!existing) {
          insertAnomaly.run(item.id, 'stock_drift', 'high', `Stock négatif: ${item.quantity}`, 0, item.quantity, null);
    ...
    try {
      ...
      const doCount = db.transaction((entries) => {
        ...
        for (const { stock_item_id, counted_qty } of entries) {
          const item = db.prepare('SELECT id, name, quantity FROM stock_items WHERE id = ?').get(stock_item_id);
          if (!item) continue;
          ...
          if (diff !== 0) {
            // Ajuster le stock
            db.prepare('UPDATE stock_items SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
              .run(counted_qty, stock_item_id);
            
            insertMovement.run(
              stock_item_id, diff, item.quantity, counted_qty,
              `Comptage inventaire (écart: ${diff > 0 ? '+' : ''}${diff})`,
              req.user?.id, req.user?.name || 'Système'
            );
            ...
            if (pct > 10) {
              insertAnomaly.run(
                ...
                item.quantity, counted_qty, Math.round(pct * 100) / 100
              );
            }
            
            results.push({ id: stock_item_id, name: item.name, previous: item.quantity, counted: counted_qty, diff });
          } else {
            results.push({ id: stock_item_id, name: item.name, previous: item.quantity, counted: counted_qty, diff: 0 });
          }
        }
        return results;
      });
  ...
  app.get('/api/inventory/stats', authenticate, (req, res) => {
    try {
      ...
      const totalItems = db.prepare('SELECT COUNT(*) as c FROM stock_items WHERE is_active = 1').get().c;
      const totalValue = db.prepare('SELECT COALESCE(SUM(unit_price * quantity), 0) as v FROM stock_items WHERE is_active = 1').get().v;
      const lowStock = db.prepare('SELECT COUNT(*) as c FROM stock_items WHERE is_active = 1 AND min_quantity > 0 AND quantity <= min_quantity').get().c;
      const zeroStock = db.prepare('SELECT COUNT(*) as c FROM stock_items WHERE is_active = 1 AND quantity = 0').get().c;
      const categories = db.prepare('SELECT COUNT(*) as c FROM stock_categories').get().c;
      const locations = db.prepare('SELECT COUNT(*) as c FROM inventory_locations WHERE is_active = 1').get().c;
      const openAnomalies = db.prepare("SELECT COUNT(*) as c FROM inventory_anomalies WHERE status = 'open'").get().c;
      const priceEntries = db.prepare('SELECT COUNT(*) as c FROM inventory_price_history').get().c;
      
      // Mouvements derniers 30 jours
      const movements30d = db.prepare(`
      ...
      const stats = {
        summary: { totalItems, totalValue: Math.round(totalValue * 100) / 100, lowStock, zeroStock, categories, locations, openAnomalies, priceEntries },
        movements30d,
        topByValue,
        abcDistribution,
        byDepot,
    ...
    try {
      ...
      const classify = db.transaction(() => {
        ...
        for (const item of items) {
          ...
          const cls = pct <= 0.80 ? 'A' : pct <= 0.95 ? 'B' : 'C';
          update.run(cls, item.id);
          if (cls === 'A') a++; else if (cls === 'B') b++; else c++;
        }
  ...
  app.get('/api/inventory/export/csv', authenticate, (req, res) => {
    try {
      ...
      const headers = ['Référence', 'Nom', 'Catégorie', 'Emplacement', 'Fournisseur', 'Quantité', 'Unité', 'Prix HT', 'Prix Vente', 'Valeur Stock', 'Seuil Min', 'Classe ABC', 'Marque', 'Modèle', 'Code-barres'];
      const rows = items.map(i => [
        i.reference || '', i.name || '', i.category_name || '', i.location_name || '', i.supplier_name || '',
        i.quantity ?? 0, i.unit || 'u', i.unit_price ?? 0, i.sell_price ?? 0,
        Math.round((i.unit_price || 0) * (i.quantity || 0) * 100) / 100,
        i.min_quantity ?? 0, i.abc_class || 'C', i.brand || '', i.model || '', i.barcode || '',
      ]);
      ...
      res.setHeader('Content-Disposition', `attachment; filename="inventaire_${new Date().toISOString().slice(0,10)}.csv"`);
  ...
  app.get('/api/inventory/export/json', authenticate, (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="inventaire_${new Date().toISOString().slice(0,10)}.json"`);
