    ...
    ? (() => {
        ...
        return {
          ...
          fieldCoverage: Object.fromEntries(
            FIELD_KEYS.map((k) => [
              k,
              success > 0 ? Math.round((fieldCoverage[k] / success) * 100) : 0,
            ]),
          ),
        };
      })()
    ...
    >
      ...
      {running && (
        <div className="bl-batch-progress-track">
          <div
            className="bl-batch-progress-fill"
            style={{
      ...
      {stats && (
        <div className="bl-batch-stats">
          ...
          <div className="bl-batch-kpi-grid">
            ...
            <KPICard
              label="Champs moy."
              value={`${stats.avgFieldsFound}/${stats.avgFieldsTotal}`}
              color="#8b5cf6"
            />
          </div>
              ...
              return (
                <div key={k} className="bl-batch-field-row">
                  ...
                  <div className="bl-batch-field-bar-track">
                    <div
                      className="bl-batch-field-bar-fill"
                      style={{ width: `${pct}%`, background: barColor }}
                    />
                  </div>
                  <span className="bl-batch-field-pct" style={{ color: barColor }}>
                    {pct}%
                  </span>
                </div>
              );
        ...
        <div>
          ...
          {results.map((r, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`bl-batch-result-card ${r.error ? 'bl-batch-result-card--error' : 'bl-batch-result-card--ok'}`}
              >
                {/* Ligne résumé */}
                <div
                ...
                >
                  ...
                  <FileText
                    ...
                    style={{ color: r.error ? STATUS_COLORS.danger : STATUS_COLORS.info }}
                  />
                  <span className="bl-batch-result-filename">{r.file.name}</span>
                  {r.error ? (
                    ...
                    <>
                      <span className="bl-batch-result-doctype">{r.docTypeLabel}</span>
                      <span
                        className="u-font-xs"
                        style={{
                          color: r.confidence >= 70 ? STATUS_COLORS.success : STATUS_COLORS.warning,
                        }}
                      >
                        {r.confidence}%
                      </span>
                      ...
                      {FIELD_KEYS.filter((k) => k !== 'items').map((k) => {
                        ...
                        return (
                          <div key={k} className="bl-batch-detail-field-row">
                            <span
                              className="bl-batch-detail-conf-dot"
                              style={{
                ...
                {isExpanded && r.error && <div className="bl-batch-error-detail">{r.error}</div>}
              </div>
            );
          })}
        </div>
      ...
      {!results && !running && files.length === 0 && (
        <div className="bl-batch-empty">
          ...
          <p className="bl-batch-empty-hint">
            Le rapport affichera la couverture de parsing, les types détectés et les champs
            extraits.
          </p>
        </div>
      )}
...
function KPICard({ label, value, total, color }) {
  return (
    <div className="bl-batch-kpi-card" style={{ background: `${color}08` }}>
      <div className="bl-batch-kpi-value" style={{ color }}>
        {value}
      </div>
