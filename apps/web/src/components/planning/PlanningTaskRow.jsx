    ...
    <div key={task.id} className={`task-row event-row-cols ${isGoogle ? 'google-task-row' : ''} ${isDone ? 'task-done-row' : ''} ${isHidden ? 'hidden-display' : ''}`}>
      ...
      <span
        ...
        title={[
          ...
          (task.locationAddress || task.eventLocation || linkedAffaire?.location) && '📍 ' + (task.locationAddress || task.eventLocation || linkedAffaire?.location),
          task.notes && '📝 ' + task.notes,
          (task.personFirstName || task.personLastName) && '👤 ' + [task.personFirstName, task.personLastName].filter(Boolean).join(' '),
        ].filter(Boolean).join('\n')}
      >
