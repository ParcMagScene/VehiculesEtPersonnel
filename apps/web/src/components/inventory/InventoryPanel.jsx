  ...
  return (
    <div className="inv-dashboard">
      ...
      <div className="inv-actions-bar">
        <Button variant="ghost" className="inv-btn" onClick={onRefresh}>
        ...
        <div className="inv-section">
          ...
          <div className="inv-abc-grid">
            {stats.abcDistribution.map((d) => (
              <div
                key={d.abc_class || 'null'}
                className={`inv-abc-card abc-${(d.abc_class || 'c').toLowerCase()}`}
              >
                <span className="inv-abc-letter">{d.abc_class || '?'}</span>
                <span>{d.count} articles</span>
                <span>
                  {d.value.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                </span>
              </div>
            ))}
          </div>
            ...
            <tbody>
              {stats.topByValue.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <strong>
                      {item.total_value?.toLocaleString('fr-FR', {
                        style: 'currency',
                        currency: 'EUR',
                      })}
                    </strong>
      ...
      {alerts.length > 0 && (
        <div className="inv-section">
          <h3>
            <AlertTriangle size={16} /> Alertes stock bas ({alerts.length})
          </h3>
          <Table className="inv-table">
            <thead>
              <tr>
...
function LocationsView({ locations, onCreate, onUpdate, onDelete }) {
  ...
  const resetForm = () => {
    setForm({ name: '', code: '', depot_number: 1, type: 'storage', zone: '', capacity: '' });
    setEditId(null);
    setShowForm(false);
  };
  ...
  return (
    <div className="inv-locations">
      <div className="inv-toolbar">
        <h3>
          <MapPin size={18} /> Emplacements ({locations.length})
        </h3>
        <Button
          variant="ghost"
        ...
        <form className="inv-form" onSubmit={handleSubmit}>
          <div className="inv-form-grid">
            ...
            <Select
              ...
              onChange={(e) => setForm((f) => ({ ...f, depot_number: Number(e.target.value) }))}
            >
          ...
          <div className="inv-form-actions">
            <Button variant="ghost" type="submit" className="inv-btn primary">
              {editId ? 'Modifier' : 'Créer'}
            </Button>
            <Button variant="ghost" type="button" className="inv-btn" onClick={resetForm}>
              Annuler
            </Button>
          </div>
        </form>
        ...
        <tbody>
          {locations.map((loc) => (
            <tr key={loc.id}>
              ...
              <td>
                <span className={`inv-badge ${loc.type}`}>{loc.type}</span>
              </td>
              <td>{loc.zone || '—'}</td>
              <td>{loc.capacity || '—'}</td>
              <td className="inv-actions">
                <Tooltip content="Modifier">
                  <Button variant="ghost" className="inv-btn-icon" onClick={() => startEdit(loc)}>
                    <Edit size={14} />
                  </Button>
                </Tooltip>
...
function PricesView() {
  ...
  return (
    <div className="inv-prices">
      <div className="inv-toolbar">
        <h3>
          <DollarSign size={18} /> Analyse prix
        </h3>
      ...
      {analysis?.item && (
        <div className="inv-price-result">
          <div className="inv-price-header">
            <h4>
              {analysis.item.name} <small>({analysis.item.reference || 'sans réf.'})</small>
            </h4>
            <span className="inv-price-current">
              Prix actuel : {analysis.item.current_price?.toFixed(2)} €
            </span>
          </div>
              ...
              <div className="inv-price-kpi">
                <div className="inv-kpi-mini">
                  ...
                  <span className="value recommended">
                    {analysis.analysis.recommended_price?.toFixed(2)} €
                  </span>
                </div>
                <div className="inv-kpi-mini">
                ...
                <div className="inv-kpi-mini">
                  ...
                  <span
                    className={`value confidence-${analysis.analysis.confidence >= 70 ? 'high' : analysis.analysis.confidence >= 40 ? 'medium' : 'low'}`}
                  >
            ...
            <tbody>
              {history.map((h) => (
                <tr key={h.id}>
                  ...
                  <td>
                    <span className={`inv-badge ${h.source}`}>{h.source}</span>
                  </td>
                  <td>{h.supplier_name || '—'}</td>
                  <td>
                    <strong>{h.price_ht?.toFixed(2)} €</strong>
                  </td>
                  <td>{h.reference || '—'}</td>
                </tr>
              ))}
...
function AnomaliesView({ anomalies, onDetect, onResolve }) {
  ...
  return (
    <div className="inv-anomalies">
      <div className="inv-toolbar">
        <h3>
          <AlertTriangle size={18} /> Anomalies ({anomalies.length} ouvertes)
        </h3>
        <Button variant="ghost" className="inv-btn primary" onClick={onDetect}>
          <Search size={14} /> Lancer détection
        </Button>
      </div>

      {anomalies.length === 0 && (
        <div className="inv-empty">
          <CheckCircle2 size={24} /> Aucune anomalie ouverte
        </div>
      )}
        ...
        {anomalies.map((a) => (
          <div key={a.id} className={`inv-anomaly-card severity-${a.severity}`}>
            ...
            <div className="inv-anomaly-actions">
              ...
              <Button
                variant="ghost"
                className="inv-btn small muted"
                onClick={() => onResolve(a.id, 'ignored')}
              >
...
function CountView({ onSubmitCount }) {
  ...
  return (
    <div className="inv-count">
      <div className="inv-toolbar">
        <h3>
          <ClipboardCheck size={18} /> Comptage inventaire
        </h3>
        <Button variant="ghost" className="inv-btn" onClick={addRow}>
      ...
      <div className="inv-count-grid">
        {items.map((item, i) => (
          <div key={i} className="inv-count-row">
            ...
            />
            <Button variant="ghost" className="inv-btn-icon danger" onClick={() => removeRow(i)}>
              <Trash2 size={14} />
            </Button>
          </div>
        ))}
      ...
      <Button variant="ghost" className="inv-btn primary" onClick={handleSubmit}>
        <CheckCircle2 size={14} /> Valider le comptage
      </Button>

      {result && (
        <div className="inv-count-result">
          <h4>
            Résultat : {result.counted} comptés, {result.adjustments} ajustement(s)
          </h4>
          {result.details
            ?.filter((d) => d.diff !== 0)
            .map((d) => (
...
function MovementsView() {
  ...
  return (
    <div className="inv-movements">
      <div className="inv-toolbar">
        <h3>
          <TrendingUp size={18} /> Mouvements ({filtered.length})
        </h3>
        <Select
              ...
              {filtered.map((m) => {
                ...
                return (
                  <tr key={m.id}>
                    ...
                    <td>
                      <span
                        className="inv-movement-badge"
                        style={{ background: (mt.color || '#888') + '20', color: mt.color }}
                      >
                    ...
                    <td
                      className={`inv-movement-qty ${m.type === 'out' ? 'negative' : 'positive'}`}
                    >
                      {m.type === 'out' ? '-' : '+'}
            ...
            {inv.stats?.summary && (
              <>
                ...
                {inv.stats.summary.lowStock > 0 && (
                  <span className="inv-stat-badge warning">
                    {inv.stats.summary.lowStock} stock bas
                  </span>
                )}
              </>
            )}
