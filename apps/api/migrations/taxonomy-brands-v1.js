...
export function runBrandsMigrations(db) {
  ...
  try {
    ...
    if (!already) {
      ...
      logger.info('  ✅ Migration brands_create_tables_v1: 3 tables créées (brands, brand_aliases, brand_family_mapping)');
    }
  } catch (e) {
  ...
  try {
    ...
    if (!already) {
      const BRANDS = [
        ...
        // Vidéo (12)
        ['Blackmagic Design','blackmagicdesign','https://www.blackmagicdesign.com','AU','video'],
  ...
  try {
    ...
    if (!already) {
      ...
      const getFamily = db.prepare("SELECT id FROM equipment_categories WHERE name = ? AND level = 'family'");
  ...
  try {
    ...
    if (!already) {
      ...
      db.exec('CREATE INDEX IF NOT EXISTS idx_supplier_articles_brand_id ON supplier_articles(brand_id)');
      db.exec('CREATE INDEX IF NOT EXISTS idx_equipment_catalog_brand_id ON equipment_catalog(brand_id)');
  ...
  try {
    ...
    if (!already) {
      ...
      const updateSA = db.prepare('UPDATE supplier_articles SET supplier_id = ? WHERE supplier_id = ?');
      ...
      for (const [masterId, dupId, name] of FUSIONS) {
        ...
        if (master && dup) {
          ...
          fused++;
          logger.info(`    Fusion ${name}: supplier ${dupId} → ${masterId} (${r1.changes} articles, ${r2.changes} commandes migrés)`);
        }
      }
  ...
  try {
    ...
    if (!already) {
      ...
      const renameStmt = db.prepare('UPDATE equipment_categories SET name = ? WHERE id = ? AND name = ?');
      let renamed = 0;
  ...
  try {
    ...
    if (!already) {
      ...
      const allBrands = db.prepare('SELECT name FROM brands').all().map(r => r.name);

      const fixEq = db.prepare('UPDATE equipment SET brand = ? WHERE brand = ? AND brand != ?');
      const fixSA = db.prepare('UPDATE supplier_articles SET brand = ? WHERE brand = ? AND brand != ?');
  ...
  try {
    ...
    if (!already) {
      const getFamily = db.prepare("SELECT id FROM equipment_categories WHERE name = ? AND level = 'family'");
      const getSub = db.prepare("SELECT id FROM equipment_categories WHERE name = ? AND parent_id = ?");
      const existsCat = db.prepare("SELECT id FROM equipment_categories WHERE name = ? AND parent_id = ?");
  ...
  try {
    ...
    const already = db.prepare('SELECT 1 FROM _migrations_log WHERE key = ?').get(migKey);
    if (!already) {
      const checkExists = db.prepare('SELECT id FROM taxonomy_family_mapping WHERE source_pattern = ?');
      const ins = db.prepare(
      ...
      const RULES = [
        ...
        ['l.acoustics|shure|sennheiser|dpa|yamaha|nexo|qsc|allen.+heath|lab.gruppen', 'Sonorisation', 15],
        ['martin|robe|clay.paky|avolites|showtec|chauvet|glp|ayrton|starway|juliat|ma.lighting', 'Éclairage', 15],
        ['prolyte|asd|layher|doughty|stagedex|europodium|stacco', 'Structure', 15],
        ['blackmagic|extron|barco|panasonic|sony|samsung|lg|unilumin|novastar|christie', 'Audiovisuel', 15],
  ...
  try {
    ...
    if (!already) {
      ...
      db.prepare('INSERT INTO _migrations_log (key) VALUES (?)').run(migKey);
      logger.info(`  ✅ Migration ${migKey}: ${eqLinked} equipment + ${saLinked} supplier_articles liés par brand_id`);
    }
  } catch (e) {
  ...
  try {
    ...
    const already = db.prepare('SELECT 1 FROM _migrations_log WHERE key = ?').get(migKey);
    if (!already) {
      const artefacts = new Set(['sommaire', 'titre sous sommaire', 's s e m e n t a u', 'vue arrière']);

      const rules = db.prepare(
        'SELECT source_pattern, target_family, is_regex FROM taxonomy_family_mapping ORDER BY priority DESC'
      ).all();

      const updateFamily = db.prepare('UPDATE supplier_articles SET unified_family = ? WHERE id = ?');
