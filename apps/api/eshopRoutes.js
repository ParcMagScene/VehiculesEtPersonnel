...
export function setupEshopRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/external-products', authenticateToken, (req, res) => {
    try {
  ...
  app.get('/api/external-products/:id', authenticateToken, (req, res) => {
    try {
      const product = db.prepare('SELECT * FROM external_products WHERE id = ?').get(req.params.id);
  ...
  app.post('/api/external-products', authenticateToken, (req, res) => {
    try {
  ...
  app.put('/api/external-products/:id', authenticateToken, (req, res) => {
    try {
      const { name, description, category, image_url, notes } = req.body;
      if (!name?.trim())
        return res.status(400).json({ success: false, error: 'Le nom est requis' });
  ...
  app.delete('/api/external-products/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      db.prepare('DELETE FROM external_products WHERE id = ?').run(req.params.id);
  ...
  app.post('/api/external-product-suppliers', authenticateToken, (req, res) => {
    try {
      const {
        product_id,
        supplier_id,
      ...
      if (!product_id)
        return res.status(400).json({ success: false, error: 'product_id est requis' });
      if (!supplier_name?.trim())
        return res.status(400).json({ success: false, error: 'supplier_name est requis' });
  ...
  app.put('/api/external-product-suppliers/:id', authenticateToken, (req, res) => {
    try {
      const {
        supplier_id,
        supplier_name,
      ...
      const row = db
        ...
        .get(req.params.id);
      if (!row) return res.status(404).json({ success: false, error: 'Entrée introuvable' });
  ...
  app.delete('/api/external-product-suppliers/:id', authenticateToken, (req, res) => {
    try {
      db.prepare('DELETE FROM external_product_suppliers WHERE id = ?').run(req.params.id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get('/api/external-products/:id/compare', authenticateToken, (req, res) => {
    try {
      const product = db.prepare('SELECT * FROM external_products WHERE id = ?').get(req.params.id);
      if (!product) return res.status(404).json({ success: false, error: 'Produit introuvable' });
  ...
  app.post('/api/external-products/quote-pdf', authenticateToken, async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="devis-eshop-${Date.now()}.pdf"`);
      ...
      if (items.length === 0) {
        ...
        items.forEach((item, idx) => {
          ...
          doc
            ...
            .text(item.price_ht != null ? item.price_ht.toFixed(2) + ' €' : '—', COL[4], rowY + 4, {
              width: 50,
              align: 'right',
            })
            .text(item.shipping != null ? item.shipping.toFixed(2) + ' €' : '—', COL[5], rowY + 4, {
              width: 50,
              align: 'right',
            })
            .text(item.total_ht != null ? item.total_ht.toFixed(2) + ' €' : '—', COL[6], rowY + 4, {
              width: 60,
              align: 'right',
            });
