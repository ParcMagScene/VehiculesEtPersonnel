...
export function setupGoogleRoutes(app, authenticateToken) {
  ...
  app.get('/api/google/callback', async (req, res) => {
    ...
    try {
      ...
      if (!result.refresh_token) {
        logger.error('[Google] Pas de refresh_token reçu — vérifiez access_type=offline et prompt=consent');
      ...
      logger.info(`[Google] Connexion réussie pour user ${pending.userId} (${result.email || '?'})`);
  ...
  app.delete('/api/google/disconnect', authenticateToken, gcalRoute(async (req, res) => {
  ...
  app.get('/api/google/calendars', authenticateToken, gcalRoute(async (req, res) => {
    await googleProxyV2(req, res, 'GET', `${GOOGLE_API_BASE}/users/me/calendarList`);
  }));

  // Ajouter un calendrier
  app.post('/api/google/calendars', authenticateToken, gcalRoute(async (req, res) => {
    await googleProxyV2(req, res, 'POST', `${GOOGLE_API_BASE}/users/me/calendarList`, req.body);
  }));

  // Lister les événements
  app.get('/api/google/events', authenticateToken, gcalRoute(async (req, res) => {
    const calendarId = getCalendarId(req);
    const params = new URLSearchParams();
    for (const key of ['timeMin', 'timeMax', 'singleEvents', 'maxResults', 'orderBy', 'q', 'pageToken', 'syncToken']) {
  ...
  app.get('/api/google/events/:eventId', authenticateToken, gcalRoute(async (req, res) => {
  ...
  // Créer un événement
  app.post('/api/google/events', authenticateToken, gcalRoute(async (req, res) => {
    const calendarId = getCalendarId(req);
  ...
  app.patch('/api/google/events/:eventId', authenticateToken, gcalRoute(async (req, res) => {
    if (!validateEventId(req, res)) return;
    const calendarId = getCalendarId(req);
    const url = `${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(req.params.eventId)}`;
  ...
  app.delete('/api/google/events/:eventId', authenticateToken, gcalRoute(async (req, res) => {
    if (!validateEventId(req, res)) return;
    const calendarId = getCalendarId(req);
    const url = `${GOOGLE_API_BASE}/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(req.params.eventId)}`;
