...
import {
  ChevronDown, Plus, Check, X, Edit2, Trash2,
  Link, RefreshCw, Repeat, AlertCircle
} from 'lucide-react';
...
const PlanningDaySection = React.memo(function PlanningDaySection({
  ...
  return (
    <div className={`task-section ${isRdv ? 'rdv-section' : ''} ${isEvenements ? 'evenements-section' : ''} ${isCollapsed ? 'section-collapsed' : ''}`}>
      <div
        ...
        <div className="ical-form">
          ...
          <div className="ical-form-row">
            ...
            <div className="form-actions">
              <Button variant="success" size="sm" iconOnly onClick={onSave} aria-label="Enregistrer"><Check size={14} /></Button>
        ...
        {icalCalendars.map(cal => (
          <div key={cal.id} className={`ical-calendar-item ${cal.lastSyncError ? 'has-error' : ''}`}>
            ...
            <div className="ical-cal-actions">
              ...
              <Tooltip content="Supprimer"><Button variant="ghost" className="delete" onClick={() => onDelete(cal.id)}><Trash2 size={13} /></Button></Tooltip>
            </div>
          </div>
        ))}
      </div>
        ...
        {recurringTasks.map(rt => (
          <div key={rt.id} className={`recurring-item ${rt.active ? '' : 'inactive'}`}>
            ...
            <div className="recurring-item-actions">
              ...
              <Tooltip content="Supprimer"><Button variant="ghost" className="delete" onClick={() => onDelete(rt.id)}><Trash2 size={14} /></Button></Tooltip>
            </div>
          </div>
        ))}
      </div>
