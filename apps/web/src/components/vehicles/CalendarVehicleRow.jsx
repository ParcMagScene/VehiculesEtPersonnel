...
const CalendarVehicleRow = ({
  ...
  const getSlotPeriod = (period, fallback) => {
  ...
  const viewStartTs =
    timeSlots.length > 0
      ? getPeriodTimestamp(timeSlots[0].day, getSlotPeriod(timeSlots[0].period, 'AM'))
      : 0;
  const lastSlot = timeSlots.length > 0 ? timeSlots[timeSlots.length - 1] : null;
  const viewEndTs = lastSlot
    ? getPeriodTimestamp(lastSlot.day, getSlotPeriod(lastSlot.period, 'PM'))
    : 0;

  const getOverflowFlags = (block) => {
    if (view === 'year' || !block?.date) {
      return { startsBeforeView: false, endsAfterView: false };
    }

    const blockStartTs = getPeriodTimestamp(block.date, block.period || 'AM');
    const blockEndTs = getPeriodTimestamp(
      block.endDate || block.date,
      block.endPeriod || block.period || 'PM',
    );

    return {
      startsBeforeView: blockStartTs < viewStartTs,
      endsAfterView: blockEndTs > viewEndTs,
    };
  };
  ...
  return (
    <div className="vehicle-row">
      {timeSlots.map((slot, slotIndex) => {
        ...
        const isInBlock = blocks.some(
          (b) => slotIndex > b.startIndex && slotIndex < b.startIndex + b.span,
        );
        ...
        if (block) {
          ...
          const { startsBeforeView, endsAfterView } = getOverflowFlags(block);
          ...
          return (
            <div
              ...
              onMouseDown={(e) => {
                if (
                  e.target.closest(
                    '.tournee-link-btn, .tournee-trip-btn, .reservation-trip-btn, .resize-handle, .reservation-delete-btn',
                  )
                )
                  return;
              ...
              onMouseMove={(e) => {
                ...
                if (
                  (blockDragState && blockDragState.vehicle.id !== vehicle.id) ||
                  (pendingBlockDragRef.current &&
                    pendingBlockDragRef.current.vehicle.id !== vehicle.id)
                )
                  return;
                ...
                const colOffset = Math.max(
                  0,
                  Math.min(block.span - 1, Math.floor(relX / colWidth)),
                );
                const slotBase = new Date(slot.day);
            ...
            >
              <div
                className={`reservation u-relative ${isBeingResized ? 'resizing' : ''} ${isBeingDragged ? 'block-drag-ghost' : ''} ${highlightedReservationIds.includes(block.id) ? 'highlighted' : ''} ${startsBeforeView ? 'reservation-overflow-start' : ''} ${endsAfterView ? 'reservation-overflow-end' : ''} ${block.isMaintenance ? `maintenance-block maintenance-status-${block.maintenanceStatus || 'scheduled'} ${getMaintenanceConflicts(block).length > 0 ? 'maintenance-conflict' : ''}` : ''}`}
                onMouseEnter={(e) => handleTooltipShow(e, block)}
                onMouseLeave={handleTooltipHide}
                style={{
                  backgroundColor: block.isMaintenance
                    ? undefined
                    : (vehicle.displayColor || vehicle.color || STATUS_COLORS.info) + '40',
                  border: block.isMaintenance
                    ? undefined
                    : `2px solid ${vehicle.displayColor || vehicle.color || STATUS_COLORS.info}`,
                  color: 'var(--theme-text-primary)',
                }}
                title={
                  `${startsBeforeView ? 'Débute avant la vue. ' : ''}${endsAfterView ? 'Se termine après la vue.' : ''}`.trim() ||
                  undefined
                }
              >
                {block.createdBy && (
                  <div
                    ...
                    title={`Créé par ${currentUser && block.createdBy === currentUser.id ? currentUser.name : users.find((u) => u.id === block.createdBy)?.name || `Utilisateur ${block.createdBy}`}`}
                  >
                ...
                <div className="reservation-content-wrapper">
                  <div className="reservation-content">
                    ...
                    {renderReservationAffaires(
                      block,
                      googleEvents,
                      timeSlots,
                      block.startIndex,
                      calendarTripCache[block.id],
                      (eventIds, mode) => handleOpenTripFromCalendar(block, eventIds, mode),
                      () => handleTripLinked(block.id),
                    )}
                  </div>
                </div>
        ...
        return (
          <div
            ...
            onMouseDown={(e) =>
              !resizeState && handleSlotMouseDown(vehicle, slot.day, slot.period, e)
            }
            onMouseEnter={() =>
              !resizeState && handleSlotMouseEnter(vehicle, slot.day, slot.period)
            }
