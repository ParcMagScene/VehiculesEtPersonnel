...
describe('Drawer', () => {
  it('ne rend rien si open=false', () => {
    const { container } = render(<Drawer open={false} onClose={() => {}}>Contenu</Drawer>);
    expect(container).toBeEmptyDOMElement();
  });

  it('rend le contenu quand open=true', async () => {
    render(<Drawer open={true} onClose={() => {}}>Contenu drawer</Drawer>);
    expect(await screen.findByText('Contenu drawer')).toBeInTheDocument();
  });

  it('affiche le titre et le bouton fermer', async () => {
    render(<Drawer open={true} onClose={() => {}} title="Détail">body</Drawer>);
