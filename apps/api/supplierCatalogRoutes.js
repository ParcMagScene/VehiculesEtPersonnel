...
export function setupSupplierCatalogRoutes(app, authenticateToken, requireWriteAccess) {
  ...
  app.get('/api/supplier-articles', authenticateToken, (req, res) => {
    try {
      const { supplier_id, brand, family, subfamily, category, search, import_id, limit, offset } =
        req.query;
      ...
      if (brand) {
        ...
        if (/^\d+$/.test(brand)) {
          query += ' AND sa.brand_id = ?';
          params.push(parseInt(brand));
        } else {
          query += ' AND (sa.brand = ? OR b.name = ?)';
      ...
      if (import_id) {
        query += ' AND sa.import_id = ?';
        params.push(import_id);
      }
      if (search) {
        query +=
          ' AND (sa.designation LIKE ? OR sa.supplier_ref LIKE ? OR sa.brand LIKE ? OR sa.model LIKE ? OR s.name LIKE ?)';
        const s = `%${search}%`;
      ...
      query +=
        ' ORDER BY sa.supplier_id, sa.family NULLS LAST, sa.brand NULLS LAST, sa.designation';
  ...
  app.get('/api/supplier-articles/filters', authenticateToken, (req, res) => {
    try {
      ...
      `,
        ...
        .all(...params)
        .map((r) => ({ name: r.brand, id: r.brand_id }));
      const families = db
        .prepare(
          `SELECT DISTINCT family FROM supplier_articles${cond}${and} family IS NOT NULL ORDER BY family`,
        )
        .all(...params)
        .map((r) => r.family);
      const categories = db
        .prepare(
          `SELECT DISTINCT category FROM supplier_articles${cond}${and} category IS NOT NULL ORDER BY category`,
        )
        .all(...params)
        .map((r) => r.category);
  ...
  app.post(
    '/api/supplier-articles/refresh-brands',
    authenticateToken,
    requireWriteAccess,
    (req, res) => {
      try {
        ...
        const allBrandNames = [];
        try {
          db.prepare('SELECT name FROM brands WHERE is_active = 1')
            .all()
            .forEach((b) => allBrandNames.push(b.name));
          db.prepare('SELECT alias FROM brand_aliases')
            .all()
            .forEach((a) => allBrandNames.push(a.alias));
        } catch {
          /* brands table may not exist yet */
        }
        ...
        const brandRx =
          allBrandNames.length > 0
            ? new RegExp(
                '\\b(' +
                  allBrandNames.map((b) => b.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|') +
                  ')\\b',
                'i',
              )
            : null;
        ...
        const updateStmt = db.prepare(
          'UPDATE supplier_articles SET brand = ?, brand_id = ? WHERE id = ?',
        );
        let brandDetected = 0;
          ...
          for (const art of articles) {
            ...
            if (currentBrand && !algamCodeRx.test(currentBrand)) {
              ...
              if (resolved.brand_id && resolved.brand !== currentBrand) {
                ...
                db.prepare(
                  'UPDATE supplier_articles SET brand_id = ? WHERE id = ? AND brand_id IS NULL',
        ...
        invalidateBrandCache();
        logger.info(
          `🏷️ Refresh marques: ${brandDetected}/${articles.length} articles mis à jour, ${mapped} unified_family mappés`,
        );
  ...
  app.post(
    '/api/supplier-articles/import',
    authenticateToken,
    requireWriteAccess,
    validate(supplierImportSchema),
    (req, res) => {
      try {
      ...
      `);
        ...
        db.prepare('UPDATE catalog_imports SET items_count = ? WHERE id = ?').run(
          inserted + updated,
          importId,
        );
        ...
        addToHistory(
          ...
          importId,
          'import',
          { filename, supplier_id, inserted, updated, skipped },
          req.user.id,
          req.user.name,
        );

        logger.info(
          `📦 Import catalogue: ${filename} — ${inserted} insérés, ${updated} mis à jour, ${skipped} ignorés`,
        );
  ...
  app.delete('/api/supplier-articles', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      addToHistory(
        ...
        0,
        'purge',
        { deleted_articles: artCount, deleted_imports: impCount },
        req.user.id,
        req.user.name,
      );

      logger.info(
        `🗑️ Purge catalogue fournisseurs: ${artCount} articles, ${impCount} imports supprimés`,
      );
  ...
  app.delete('/api/catalog-imports/:id', authenticateToken, requireWriteAccess, (req, res) => {
    try {
      ...
      const deleteAll = db.transaction(() => {
        const del = db
          .prepare('DELETE FROM supplier_articles WHERE import_id = ?')
          .run(req.params.id);
  ...
  app.post(
    ...
    (req, res) => {
      try {
        ...
        const withDesignation = items.filter(
          (a) => a.designation && a.designation.length > 2,
        ).length;
        ...
        for (const art of items) {
          ...
          if (
            art.designation &&
            /^(page|total|sous-total|catalogue|tarif|conditions|contact|www\.|http)/i.test(
              art.designation,
            )
          ) {
  ...
  app.post(
    '/api/supplier-articles/taxonomy/apply',
    authenticateToken,
    requireWriteAccess,
    validate(taxonomyApplySchema),
    (req, res) => {
      try {
        ...
        addToHistory(
          ...
          0,
          'taxonomy_normalize',
          { rules: rules.length, totalChanged },
          req.user?.id,
          req.user?.name,
        );

        logger.info(
          `📋 Normalisation taxonomie: ${rules.length} règles, ${totalChanged} articles modifiés`,
        );
  ...
  app.post(
    ...
    (req, res) => {
      try {
        ...
        db.prepare(
          'INSERT INTO brand_aliases (brand_id, alias, alias_slug, source) VALUES (?, ?, ?, ?)',
  ...
  app.post(
    '/api/supplier-articles/link-brand-ids',
    authenticateToken,
    requireWriteAccess,
    (req, res) => {
      try {
  ...
  app.post(
    '/api/supplier-articles/apply-unified-family',
    authenticateToken,
    requireWriteAccess,
    (req, res) => {
      try {
