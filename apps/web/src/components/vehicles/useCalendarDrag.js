...
export default function useCalendarDrag({
  ...
  const handleSlotMouseEnter = (vehicle, date, period) => {
    ...
    if (pendingBlockDragRef.current) {
      ...
      if (!isSameSlot) {
        ...
        const tsToDP = (ts) => {
          const d = new Date(Math.floor(ts / 2)); d.setHours(0, 0, 0, 0);
  ...
  const handleResizeStart = (e, block, edge) => {
    ...
    const initialDate = edge === 'start' ? new Date(block.date) : new Date(block.endDate || block.date);
    ...
    setResizeState({
      reservation: block, edge,
      initialDate, initialPeriod,
      currentDay: initialDate, currentPeriod: initialPeriod,
      originalStartDate: startDate, originalStartPeriod: block.period,
      originalEndDate: endDate, originalEndPeriod: block.endPeriod || block.period
    });
    setResizePreview({
      vehicleId: block.vehicleId,
      startDate, startPeriod: block.period,
      endDate, endPeriod: block.endPeriod || block.period
    });
  };

  const handleResizeMove = (day, period) => {
    if (!resizeState) return;
    if (resizeState.currentDay && isSameDay(resizeState.currentDay, day) && resizeState.currentPeriod === period) return;

    const { edge, originalStartDate, originalStartPeriod, originalEndDate, originalEndPeriod, reservation } = resizeState;
    const normalizedDay = new Date(day); normalizedDay.setHours(0, 0, 0, 0);

    let previewStartDate, previewStartPeriod, previewEndDate, previewEndPeriod;
    if (edge === 'start') {
      previewStartDate = normalizedDay; previewStartPeriod = period;
      previewEndDate = originalEndDate; previewEndPeriod = originalEndPeriod;
    } else {
      previewStartDate = originalStartDate; previewStartPeriod = originalStartPeriod;
      previewEndDate = normalizedDay; previewEndPeriod = period;
    }

    setResizePreview({
      vehicleId: reservation.vehicleId,
      startDate: previewStartDate, startPeriod: previewStartPeriod,
      endDate: previewEndDate, endPeriod: previewEndPeriod
    });
  ...
  const handleResizeEnd = () => {
    ...
    const normalizedCurrentDay = new Date(currentDay); normalizedCurrentDay.setHours(0, 0, 0, 0);
    const hasMoved = !isSameDay(initialDate, normalizedCurrentDay) || initialPeriod !== currentPeriod;

    if (!hasMoved) { setResizeState(null); return; }
    ...
    if (edge === 'start') {
      newStartDate = normalizedCurrentDay; newStartPeriod = currentPeriod;
      newEndDate = originalEndDate; newEndPeriod = originalEndPeriod;
      if (getPeriodTimestamp(newStartDate, newStartPeriod) > getPeriodTimestamp(newEndDate, newEndPeriod)) {
        setResizeState(null); setResizePreview(null); return;
      }
    } else {
      newStartDate = originalStartDate; newStartPeriod = originalStartPeriod;
      newEndDate = normalizedCurrentDay; newEndPeriod = currentPeriod;
      if (getPeriodTimestamp(newEndDate, newEndPeriod) < getPeriodTimestamp(newStartDate, newStartPeriod)) {
        setResizeState(null); setResizePreview(null); return;
      }
    }
    ...
    onUpdateReservation(reservation.id, {
      ...reservation,
      date: formatLocalDate(newStartDate), period: newStartPeriod,
      endDate: formatLocalDate(newEndDate), endPeriod: newEndPeriod
    });
    setResizeState(null);
  ...
  const handleBlockDragMove = (vehicle, day, period) => {
    ...
    setBlockDragState(prev => ({ ...prev, targetVehicle: vehicle, currentStart: newStart, currentEnd: newEnd, hasConflict }));
    setBlockDragPreview({
      vehicleId: vehicle.id,
      startDate: newStart.date, startPeriod: newStart.period,
      endDate: newEnd.date, endPeriod: newEnd.period, hasConflict,
    });
  };

  const handleBlockDragEnd = () => {
    if (!blockDragState) return;
    const { block, vehicle, targetVehicle, currentStart, currentEnd, originalStart, hasConflict } = blockDragState;
    ...
    if ((hasMoved || hasChangedVehicle) && !hasConflict) {
      ...
      if (existing) {
        onUpdateReservation(existing.id, {
          ...existing,
          date: formatLocalDate(currentStart.date), period: currentStart.period,
          endDate: formatLocalDate(currentEnd.date), endPeriod: currentEnd.period,
  ...
  const handleSlotClick = (vehicle, date, period) => {
    ...
    if (existing) { onSelectReservation(existing); } else { onSelectSlot({ vehicle, date, period }); }
  };
  ...
  const isInDragSelection = useCallback((vehicleId, date, period) => {
    ...
    const allSlots = [];
    days.forEach(day => { dragPeriods.forEach(p => { allSlots.push({ day, period: p }); }); });
    const startIndex = allSlots.findIndex(s => isSameDay(s.day, dragState.startDay) && s.period === dragState.startPeriod);
    const endIndex = allSlots.findIndex(s => isSameDay(s.day, dragState.endDay) && s.period === dragState.endPeriod);
  ...
  const handleGlobalMouseMove = (e) => {
    ...
    if (lastPositionRef.current.dayIndex === dayIndex && lastPositionRef.current.period === period) return;
  ...
  useEffect(() => {
    const onDocumentMouseUp = () => {
      ...
      if (blockDragState) { handleBlockDragEnd(); return; }
      if (isDragging) { handleSlotMouseUp(); }
  ...
  return {
    ...
    isInResizePreview, handleResizeStart, handleResizeMove, handleResizeEnd,
    handleBlockMouseDown, handleBlockDragMove, handleBlockDragEnd,
