...
// ===============================================
//  SURVEILLANCE VIDÉO — WebRTC (TV)
// ===============================================
let tvCameras = [];
let tvVideoConnections = {};  // { cameraId: { pc, streamId } }
let tvVideoGrid = 4;          // Grid layout: 1, 4, 9, 16
let tvVideoRotationTimer = null;
let tvVideoCurrentPage = 0;
let tvNightMode = false;

async function loadTVCameras() {
  try {
    const response = await fetch(`${API_BASE}/api/video/tv/cameras`);
    if (!response.ok) return;
    tvCameras = await response.json();
    if (tvCameras.length > 0) {
      renderVideoGrid();
    }
  } catch (error) {
    console.error('Erreur chargement caméras TV:', error);
  }
}
...
function renderVideoGrid() {
  ...
  // Déterminer le grid layout selon le nombre de caméras
  const total = tvCameras.length;
  if (total <= 1) tvVideoGrid = 1;
  else if (total <= 4) tvVideoGrid = 4;
  else if (total <= 9) tvVideoGrid = 9;
  else tvVideoGrid = 16;

  // Pagination : caméras visibles sur la page courante
  const perPage = tvVideoGrid;
  const totalPages = Math.ceil(total / perPage);
  tvVideoCurrentPage = tvVideoCurrentPage % Math.max(totalPages, 1);
  const start = tvVideoCurrentPage * perPage;
  const visibleCameras = tvCameras.slice(start, start + perPage);

  // Nettoyer les connexions des caméras plus visibles
  const visibleIds = new Set(visibleCameras.map(c => c.id));
  for (const [camId, conn] of Object.entries(tvVideoConnections)) {
    if (!visibleIds.has(Number(camId))) {
      cleanupVideoConnection(camId);
    }
  }

  // Calculer les colonnes CSS
  const cols = Math.ceil(Math.sqrt(tvVideoGrid));
  grid.style.gridTemplateColumns = `repeat(${cols}, 1fr)`;
  grid.innerHTML = '';

  visibleCameras.forEach(camera => {
    const cell = document.createElement('div');
    cell.className = 'tv-video-cell';
    ...
    video.className = 'tv-video-player';

    const label = document.createElement('div');
    label.className = 'tv-video-label';
    label.textContent = camera.name || `Caméra ${camera.id}`;

    const statusDot = document.createElement('div');
    statusDot.className = `tv-video-dot ${camera.status === 'online' ? 'online' : 'offline'}`;
    ...
    connectTVCamera(camera.id, video);
  });

  if (statusEl) {
    statusEl.textContent = totalPages > 1
      ? `Page ${tvVideoCurrentPage + 1}/${totalPages} • ${total} caméra${total > 1 ? 's' : ''}`
      : `${total} caméra${total > 1 ? 's' : ''}`;
  }
}

async function connectTVCamera(cameraId, videoElement) {
  // Nettoyer une connexion existante
  cleanupVideoConnection(cameraId);

  try {
    // 1. Récupérer l'offer WebRTC
    const offerResp = await fetch(`${API_BASE}/api/video/tv/cameras/${cameraId}/webrtc-offer`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    });
    if (!offerResp.ok) {
      markCameraError(cameraId, videoElement);
      return;
    }
    const { offer, streamId } = await offerResp.json();

    // 2. Créer le RTCPeerConnection
    ...
    pc.oniceconnectionstatechange = () => {
      if (pc.iceConnectionState === 'failed' || pc.iceConnectionState === 'disconnected') {
        ...
        setTimeout(() => {
          const cell = videoElement.closest('.tv-video-cell');
          if (cell) connectTVCamera(cameraId, videoElement);
        }, 5000);
      }
    };

    // 3. Appliquer l'offer et créer l'answer
    ...
    // 4. Envoyer l'answer
    await fetch(`${API_BASE}/api/video/tv/cameras/${cameraId}/webrtc-answer`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ streamId, answer: answer.sdp }),
    });

    tvVideoConnections[cameraId] = { pc, streamId };
  } catch (error) {
    console.error(`Erreur WebRTC caméra ${cameraId}:`, error);
    markCameraError(cameraId, videoElement);
  }
}

function markCameraError(cameraId, videoElement) {
  const cell = videoElement.closest('.tv-video-cell');
  if (cell) {
    cell.classList.add('error');
    const dot = cell.querySelector('.tv-video-dot');
    if (dot) { dot.className = 'tv-video-dot offline'; }
  }
}

function cleanupVideoConnection(cameraId) {
  const conn = tvVideoConnections[cameraId];
  if (conn) {
    try { conn.pc.close(); } catch (_) { /* ignore */ }
    delete tvVideoConnections[cameraId];
  }
}

function cleanupAllVideoConnections() {
  for (const camId of Object.keys(tvVideoConnections)) {
    cleanupVideoConnection(camId);
  }
}

// Rotation automatique entre les pages
function startVideoRotation(intervalMs) {
  stopVideoRotation();
  if (tvCameras.length <= tvVideoGrid) return; // Pas de rotation si tout tient sur une page
  tvVideoRotationTimer = setInterval(() => {
    const totalPages = Math.ceil(tvCameras.length / tvVideoGrid);
    tvVideoCurrentPage = (tvVideoCurrentPage + 1) % totalPages;
    renderVideoGrid();
  }, intervalMs || 30000);
}

function stopVideoRotation() {
  if (tvVideoRotationTimer) {
    clearInterval(tvVideoRotationTimer);
    tvVideoRotationTimer = null;
  }
}

// Mode nuit : réduit la luminosité
function toggleNightMode() {
  tvNightMode = !tvNightMode;
  const grid = document.getElementById('video-grid');
  if (grid) {
    grid.classList.toggle('night-mode', tvNightMode);
  }
}
...
function checkAutoNightMode() {
  ...
  if (shouldBeNight !== tvNightMode) {
    tvNightMode = shouldBeNight;
    const grid = document.getElementById('video-grid');
    if (grid) grid.classList.toggle('night-mode', tvNightMode);
  }
}
