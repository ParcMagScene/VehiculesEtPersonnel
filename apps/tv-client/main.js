...
function applyConfig(config) {
  ...
  // Priorite : query string ?tvScale=X > config serveur > default 1.0 (taille native).
 ...
 * Valeur par defaut : 1.0 (taille native, aucun zoom). Passer a 1.6-1.8 pour TV 65" a 4-5 m
 * via ?tvScale=X ou display_config.tvScale.
...
function applyTvScale(raw) {
  ...
  const scale = Number.isFinite(parsed) && parsed >= 0.5 && parsed <= 3 ? parsed : 1.0;
