...
function MobileLocation({ onBack }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [zonesData, statsData] = await Promise.all([
        fetch(`/depot${depot === 1 ? '' : depot}-zones.json`)
          .then((r) => r.json())
          .catch(() => null),
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) setZoneEquipments([]);
      }
  ...
  const getZoneCount = useCallback(
    (zoneId) => {
  ...
  const unlocated = stats.find(
    (s) => (s.location_zone || s.locationZone) === '' || s.location_zone === null,
  );
    ...
    <div className="mobile-location" {...ptrProps}>
      ...
      {searchResults && (
        <div className="mloc-search-results">
          <div className="mloc-search-count">
            ...
            return (
              <div key={zone} className="mloc-search-group">
                ...
                <div className="mloc-search-items">
                  ...
                  {eqs.length > 5 && (
                    <div className="mloc-search-more">+{eqs.length - 5} autres</div>
                  )}
                </div>
              </div>
            );
        ...
        <>
          ...
          >
            <div className="mloc-zoom-controls">
              <Button variant="ghost" onClick={handleZoomIn} aria-label="Zoomer">
                <ZoomIn size={18} />
              </Button>
              <Button variant="ghost" onClick={handleZoomOut} aria-label="Dézoomer">
                <ZoomOut size={18} />
              </Button>
              <Button variant="ghost" onClick={handleResetView} aria-label="Réinitialiser la vue">
                <RotateCcw size={18} />
              </Button>
            </div>
            ...
            >
              ...
              {floorZones.map((zone) => {
                ...
                const opacity = searchResults ? (hasSearchResult ? 1 : 0.3) : isSelected ? 1 : 0.85;
                ...
                const polyPoints = isPolygon
                  ? (hasClip ? zone.clipPoints : getZonePoints(zone.bbox, zone.skew))
                      .map((p) => `${p.x},${p.y}`)
                      .join(' ')
                  : null;
          ...
          {(zones?.zones || [])
            ...
            .map((zone) => {
              ...
              return (
                ...
                >
                  <div
                    className="mloc-zone-color"
                    style={{ background: zone.color || STATUS_COLORS.neutralSoft }}
                  />
                  <div className="mloc-zone-info">
                    <div className="mloc-zone-name">{zone.label}</div>
                    <div className="mloc-zone-floor">
                      {zone.floor} · {zone.id}
                    </div>
                  </div>
      ...
      {selectedZone && (
        <div className="mloc-equipment-panel">
          <div className="mloc-panel-header">
            <div>
              <h3>{zones?.zones?.find((z) => z.id === selectedZone)?.label || selectedZone}</h3>
              <span className="mloc-panel-count">
                {zoneEquipments.length} équipement{zoneEquipments.length > 1 ? 's' : ''}
              </span>
            </div>
            <Button variant="ghost" onClick={() => setSelectedZone(null)} aria-label="Fermer">
              <X size={20} />
            </Button>
          </div>
