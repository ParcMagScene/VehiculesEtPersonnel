      ...
      {step === 'upload' && (
        <div className="eq-import-upload">
          ...
          >
            ...
            <Button
              variant="primary"
              onClick={() => document.getElementById('personnel-csv-file-input').click()}
            >
      ...
      {step === 'preview' && csvData && (
        <div className="pi-import-preview">
          ...
          {preview && (
            <div className="eq-import-section">
              <h4>
                <AlertCircle size={14} /> Analyse des collisions
              </h4>
                ...
                <Accordion title={`Détail (${filteredAnalysis.length} entrées)`}>
                  <div className="pi-collision-list">
                          ...
                          <tr key={i} className={`pi-row-${entry.action}`}>
                            <td>
                              ...
                              >
                                {entry.action === 'create' && (
                                  <>
                                    <UserPlus size={12} /> Créer
                                  </>
                                )}
                                {entry.action === 'update' && (
                                  <>
                                    <RefreshCw size={12} /> MAJ
                                  </>
                                )}
                            ...
                            <td>
                              {entry.collision ? (
                                <span className="pi-collision-match">
                                  ...
                                  <span className="pi-collision-via">
                                    {' '}
                                    (via{' '}
                                    {entry.collision.collisionType === 'code_libre'
                                      ? 'code'
                                      : 'nom'}
                                    )
                                  </span>
                    ...
                    {filteredAnalysis.length > 50 && (
                      <p className="eq-import-more">
                        ... et {filteredAnalysis.length - 50} entrées supplémentaires
                      </p>
                    )}
                  </div>
                </Accordion>
          ...
          {!preview && (
            <div className="pi-analyze-cta">
              <Button variant="primary" onClick={handlePreview} disabled={loading}>
                {loading ? (
                  <>
                    <Spinner size={14} /> Analyse...
                  </>
                ) : (
                  <>
                    <AlertCircle size={14} /> Analyser les collisions
                  </>
                )}
              </Button>
