...
function buildSearchQuery(baseTable, searchFields, req) {
  ...
  const allowedSorts = ['name', 'code_libre', 'city', 'created_at', 'modified_at', 'postal_code', 'last_name'];
...
export function setupAnnuaireClientsRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/annuaire/clients', authenticateToken, (req, res) => {
    try {
      ...
      const { where, params, sortCol, sortOrder, limit, offset } = buildSearchQuery('clients', searchFields, req);
  ...
  app.post('/api/annuaire/clients', authenticateToken, (req, res) => {
    try {
      ...
      addToHistory('client', result.lastInsertRowid, 'created', req.body, req.user.id, req.user.name);
  ...
  app.delete('/api/annuaire/clients/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      const contactCount = db.prepare('SELECT COUNT(*) as c FROM annuaire_contacts WHERE client_id = ?').get(req.params.id);
      if (contactCount.c > 0) {
        // Soft delete
        db.prepare('UPDATE clients SET is_active = 0, modified_by = ?, modified_at = CURRENT_TIMESTAMP WHERE id = ?')
...
export function setupAnnuaireSuppliersRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/annuaire/suppliers', authenticateToken, (req, res) => {
    try {
      const searchFields = ['name', 'code_libre', 'contact_name', 'email', 'phone', 'city', 'postal_code'];
      const { where, params, sortCol, sortOrder, limit, offset } = buildSearchQuery('suppliers', searchFields, req);
  ...
  app.post('/api/annuaire/suppliers', authenticateToken, (req, res) => {
    try {
      ...
      addToHistory('supplier', result.lastInsertRowid, 'created', req.body, req.user.id, req.user.name);
  ...
  app.delete('/api/annuaire/suppliers/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      const orderCount = db.prepare('SELECT COUNT(*) as c FROM orders WHERE supplier_id = ?').get(req.params.id);
      const contactCount = db.prepare('SELECT COUNT(*) as c FROM annuaire_contacts WHERE supplier_id = ?').get(req.params.id);
      if (orderCount.c > 0 || contactCount.c > 0) {
        db.prepare('UPDATE suppliers SET is_active = 0, modified_by = ?, modified_at = CURRENT_TIMESTAMP WHERE id = ?')
          .run(req.user.id, req.params.id);
      } else {
...
export function setupAnnuairePrestatairesRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/annuaire/prestataires', authenticateToken, (req, res) => {
    try {
      const searchFields = ['name', 'code_libre', 'email', 'phone', 'city', 'postal_code', 'activity_sector'];
      const { where, params, sortCol, sortOrder, limit, offset } = buildSearchQuery('prestataires', searchFields, req);

      const countRow = db.prepare(`SELECT COUNT(*) as total FROM prestataires${where}`).get(...params);
      const rows = db.prepare(
        `SELECT p.*,
  ...
  app.post('/api/annuaire/prestataires', authenticateToken, (req, res) => {
    try {
      ...
      addToHistory('prestataire', result.lastInsertRowid, 'created', req.body, req.user.id, req.user.name);
      const created = db.prepare('SELECT * FROM prestataires WHERE id = ?').get(result.lastInsertRowid);
      res.status(201).json(created);
    } catch (error) {
  ...
  app.delete('/api/annuaire/prestataires/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      const contactCount = db.prepare('SELECT COUNT(*) as c FROM annuaire_contacts WHERE prestataire_id = ?').get(req.params.id);
      if (contactCount.c > 0) {
        db.prepare('UPDATE prestataires SET is_active = 0, modified_by = ?, modified_at = CURRENT_TIMESTAMP WHERE id = ?')
          .run(req.user.id, req.params.id);
      } else {
...
export function setupAnnuaireContactsRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/annuaire/contacts', authenticateToken, (req, res) => {
    try {
      ...
      if (search) {
        conditions.push("(c.last_name LIKE ? OR c.first_name LIKE ? OR c.email LIKE ? OR c.phone LIKE ? OR c.job_title LIKE ?)");
        for (let i = 0; i < 5; i++) params.push(`%${search}%`);
      }
      if (client_id) { conditions.push('c.client_id = ?'); params.push(client_id); }
      if (supplier_id) { conditions.push('c.supplier_id = ?'); params.push(supplier_id); }
      if (prestataire_id) { conditions.push('c.prestataire_id = ?'); params.push(prestataire_id); }

      const where = conditions.length ? ' WHERE ' + conditions.join(' AND ') : '';
      const offset = (Math.max(1, parseInt(page)) - 1) * parseInt(limit);

      const countRow = db.prepare(`SELECT COUNT(*) as total FROM annuaire_contacts c${where}`).get(...params);
      const rows = db.prepare(`
        SELECT c.*,
  ...
  app.post('/api/annuaire/contacts', authenticateToken, (req, res) => {
    try {
      ...
      const result = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
      `).run(client_id || null, supplier_id || null, prestataire_id || null,
             first_name || null, last_name, job_title || null, category || null,
             email || null, phone || null, phone2 || null, is_primary ? 1 : 0, notes || null,
             req.user.id, req.user.id);

      const created = db.prepare('SELECT * FROM annuaire_contacts WHERE id = ?').get(result.lastInsertRowid);
      res.status(201).json(created);
    } catch (error) {
  ...
  app.put('/api/annuaire/contacts/:id', authenticateToken, (req, res) => {
    try {
      const { client_id, supplier_id, prestataire_id, first_name, last_name, job_title,
              category, email, phone, phone2, is_primary, notes, is_active } = req.body;
...
export function setupAnnuaireLookupsRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.get('/api/annuaire/ref/all', authenticateToken, (req, res) => {
    try {
      const result = {};
      for (const [slug, table] of Object.entries(lookupTables)) {
        result[slug.replace(/-/g, '_')] = db.prepare(`SELECT * FROM ${table} WHERE is_active = 1 ORDER BY sort_order ASC, name ASC`).all();
      }
      res.json(result);
    } catch (error) {
...
export function setupAnnuaireSearchRoutes(app, authenticateToken) {
  ...
  app.get('/api/annuaire/search', authenticateToken, (req, res) => {
    try {
      ...
      if (!q || q.length < 2) return res.json({ clients: [], suppliers: [], prestataires: [], contacts: [] });
...
export function setupAnnuaireImportRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.post('/api/annuaire/import/clients-csv', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      const importTransaction = db.transaction(() => {
        for (const line of dataLines) {
          try {
            ...
            if (!codeFree || !name) { skipped++; continue; }
            ...
            insertOrUpdate.run(codeFree, name, cp || null, ville || null, phone, commentaire || null);
            imported++;
          } catch (e) {
            errors++;
          }
      ...
      importTransaction();
      logger.info(`Import clients CSV: ${imported} importés, ${skipped} ignorés, ${errors} erreurs`);
  ...
  app.post('/api/annuaire/import/suppliers-csv', authenticateToken, requireAdmin, (req, res) => {
    try {
          ...
          try {
            ...
            for (let i = 0; i < values.length; i++) {
              ...
              if (phase === 'name') {
                ...
                if (/^0[1-9]$/.test(val) && i + 3 < values.length && values.slice(i, i + 5).every(v => /^\d{2}$/.test(v))) {
      ...
      importTransaction();
      logger.info(`Import fournisseurs CSV: ${imported} importés, ${skipped} ignorés, ${errors} erreurs`);
      res.json({ success: true, imported, skipped, errors, total: dataLines.length });
    } catch (error) {
