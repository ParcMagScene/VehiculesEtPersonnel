...
const MobileReservations = forwardRef(
  (
    {
      vehicles,
      reservations,
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (currentUser?.isAdmin) {
          ...
          toast.success(
            'Demande de réservation envoyée ! Votre demande a été transmise aux administrateurs pour validation.',
          );
        }
        setShowForm(false);
        setFormData({
    ...
    const availableVehicles = vehicles.filter((v) => {
      ...
      const hasConflict = reservations.some(
        (r) => r.vehicleId === v.id && new Date(r.startDate) <= end && new Date(r.endDate) >= start,
      );
                ...
                {formData.vehicleId ? (
                  (() => {
                    const v = availableVehicles.find(
                      (veh) => String(veh.id) === String(formData.vehicleId),
                    );
                    if (!v)
                      return (
