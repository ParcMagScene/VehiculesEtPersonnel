...
function cleanTvTitle(t) {
  ...
  if (!title)
    title =
      (t.affaire_nom || '').trim() ||
      (t.affaire_titre || '').trim() ||
      (t.affaire_client || '').trim() ||
      (t.event_client || '').trim() ||
      t.notes ||
      '-';
  // Auto-majuscule
...
function readJsonFile(filePath, fallback = {}) {
  try {
    ...
    logger.warn('JSON read error:', e.message);
  }
  return fallback;
}
...
export function setupDisplayRoutes(app, authenticateToken, requireAdmin) {
  ...
  app.post(
    ...
    (req, res) => {
      ...
      `,
        ...
        const screen = db
          .prepare('SELECT * FROM display_screens WHERE id = ?')
  ...
  app.put(
    '/api/display/screens/:id',
    authenticateToken,
    requireAdmin,
    validate(screenUpdateSchema),
    (req, res) => {
      try {
  ...
  app.get('/api/display/playlists/:id', authenticateToken, (req, res) => {
    try {
      const playlist = db
        .prepare('SELECT * FROM display_playlists WHERE id = ?')
        .get(req.params.id);
    ...
    (req, res) => {
      ...
      `,
        ...
        logAction(
          null,
          'playlist_updated',
          { id: req.params.id, name: name ?? existing.name },
          req.user.id,
        );
  ...
  app.delete('/api/display/playlists/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      db.prepare('UPDATE display_screens SET playlist_id = NULL WHERE playlist_id = ?').run(
        req.params.id,
      );
  ...
  app.put(
    '/api/display/playlists/:id/items',
    authenticateToken,
    requireAdmin,
    validate(playlistItemsSchema),
    (req, res) => {
      try {
        ...
        logAction(
          ...
          { id: req.params.id, count: items.length },
          req.user.id,
        );
        ...
        const updatedItems = db
          .prepare('SELECT * FROM display_playlist_items WHERE playlist_id = ? ORDER BY sort_order')
          .all(req.params.id);
  ...
  app.post(
    ...
    validateFileType(DISPLAY_MEDIA_MIMES),
    (req, res) => {
      try {
        if (!req.file)
      ...
      `,
        ...
        const media = db
          .prepare('SELECT * FROM display_media WHERE id = ?')
          .get(result.lastInsertRowid);
  ...
  app.delete('/api/display/media/:id', authenticateToken, (req, res) => {
    try {
      ...
      db.prepare('DELETE FROM display_playlist_items WHERE item_type = ? AND item_id = ?').run(
        'media',
        req.params.id,
      );
      ...
      logAction(
        null,
        'media_deleted',
        { id: req.params.id, filename: media.original_name },
        req.user.id,
      );
      res.json({ success: true });
    } catch (error) {
  ...
  app.post('/api/display/messages', authenticateToken, validate(messageSchema), (req, res) => {
    try {
      ...
      const message = db
        .prepare('SELECT * FROM display_messages WHERE id = ?')
        .get(result.lastInsertRowid);
    ...
    (req, res) => {
      ...
      `,
        ...
        logAction(
          null,
          'message_updated',
          { id: req.params.id, title: title ?? existing.title },
          req.user.id,
        );
  ...
  app.get('/api/display/templates', authenticateToken, (_req, res) => {
    try {
      const templates = db
        .prepare('SELECT * FROM display_templates WHERE is_active = 1 ORDER BY name')
        .all();
      res.json(templates);
    } catch (error) {
  ...
  app.post(
    ...
    (req, res) => {
      ...
      `,
        ...
        const template = db
          .prepare('SELECT * FROM display_templates WHERE id = ?')
          .get(result.lastInsertRowid);
    ...
    (req, res) => {
      try {
        ...
        const existing = db
          ...
          .get(req.params.id);
        if (!existing)
          return res.status(404).json({ success: false, error: 'Template introuvable' });
  ...
  app.delete('/api/display/templates/:id', authenticateToken, requireAdmin, (req, res) => {
    try {
      ...
      db.prepare('UPDATE display_messages SET template_id = NULL WHERE template_id = ?').run(
        req.params.id,
      );
      db.prepare('DELETE FROM display_playlist_items WHERE item_type = ? AND item_id = ?').run(
        'template',
  ...
  app.get('/api/display/logs', authenticateToken, (req, res) => {
    try {
      ...
      params.push(
        Math.max(1, Math.min(parseInt(limit, 10) || 100, 1000)),
        Math.max(0, parseInt(offset, 10) || 0),
      );

      const logs = db.prepare(query).all(...params);
      const total = db
        .prepare(`SELECT COUNT(*) as c FROM display_logs${screenId ? ' WHERE screen_id = ?' : ''}`)
  ...
  app.get('/api/display/stats', authenticateToken, (_req, res) => {
    try {
      const screens = db
        .prepare(
          "SELECT COUNT(*) as total, SUM(CASE WHEN status = 'online' THEN 1 ELSE 0 END) as online FROM display_screens WHERE is_active = 1",
        )
        .get();
      const playlists = db
        .prepare('SELECT COUNT(*) as total FROM display_playlists WHERE is_active = 1')
        .get();
      const media = db
        .prepare(
          'SELECT COUNT(*) as total, SUM(file_size) as totalSize FROM display_media WHERE is_active = 1',
        )
        .get();
      const messages = db
        .prepare(
          "SELECT COUNT(*) as total, SUM(CASE WHEN is_active = 1 AND (date_end IS NULL OR date_end >= date('now')) THEN 1 ELSE 0 END) as active FROM display_messages",
        )
        .get();
      const templates = db
        .prepare('SELECT COUNT(*) as total FROM display_templates WHERE is_active = 1')
        .get();
  ...
  app.post(
    '/api/display/appearance',
    authenticateToken,
    requireAdmin,
    validate(appearanceSchema),
    (req, res) => {
      try {
  ...
  app.post(
    ...
    authenticateToken,
    requireAdmin,
    validate(welcomeMessagesSchema),
    (req, res) => {
      try {
  ...
  app.get('/api/display/welcome-message', authenticateToken, (_req, res) => {
    try {
      ...
      let slot;
      if (timeMinutes < TIME_SLOTS.MATIN_END)
        slot = 'matin'; // avant 9h30
      else if (timeMinutes < TIME_SLOTS.MATINEE_END)
        slot = 'matinee'; // 9h30 - 12h
      else if (timeMinutes < TIME_SLOTS.MIDI_END)
        slot = 'midi'; // 12h - 13h
      else if (timeMinutes < TIME_SLOTS.APRES_MIDI_END)
        slot = 'apres_midi'; // 13h - 18h
  ...
  app.get('/api/display/sidebar-config', authenticateToken, (_req, res) => {
    try {
      const row = db
        .prepare("SELECT value FROM display_config WHERE key = 'sidebarSections'")
        .get();
  ...
  app.post(
    '/api/display/color-rules',
    authenticateToken,
    requireAdmin,
    validate(colorRulesSchema),
    (req, res) => {
      try {
        ...
        const t = db.transaction(() => {
          ...
          const insert = db.prepare(
            'INSERT INTO display_color_rules (keyword, color, description, sort_order) VALUES (?, ?, ?, ?)',
          );
          rules.forEach((r, i) => {
  ...
  app.post(
    '/api/display/location-gifs',
    authenticateToken,
    requireAdmin,
    uploadGif.single('gif'),
    validateFileType(DISPLAY_GIF_MIMES),
    (req, res) => {
      try {
  ...
  app.delete(
    '/api/display/location-gifs/:filename',
    authenticateToken,
    requireAdmin,
    (req, res) => {
      try {
  ...
  app.get('/api/display/location-icon-rules', authenticateToken, (_req, res) => {
    try {
      const rules = db
        .prepare('SELECT * FROM display_location_icon_rules ORDER BY sort_order, id')
        .all();
      res.json({ rules });
    } catch (error) {
  ...
  app.post(
    '/api/display/location-icon-rules',
    authenticateToken,
    requireAdmin,
    validate(locationIconRulesSchema),
    (req, res) => {
      try {
        const { rules } = req.body;
        ...
        const t = db.transaction(() => {
          ...
          const insert = db.prepare(
            'INSERT INTO display_location_icon_rules (keyword, gif_filename, sort_order) VALUES (?, ?, ?)',
          );
          rules.forEach((r, i) => {
  ...
  app.post(
    ...
    validateFileType(DISPLAY_IMAGE_MIMES),
    (req, res) => {
      try {
        if (!req.file) return res.status(400).json({ success: false, error: 'Fichier requis' });
  ...
  app.post(
    '/api/display/sneaky-photo',
    authenticateToken,
    requireAdmin,
    uploadSneaky.single('photo'),
    validateFileType(DISPLAY_IMAGE_MIMES),
    (req, res) => {
      try {
  ...
  app.post(
    '/api/display/sneaky-message',
    authenticateToken,
    requireAdmin,
    validate(sneakyMessageSchema),
    (req, res) => {
      try {
  ...
  app.get('/api/display/tv-state', authenticateToken, async (req, res) => {
    try {
      ...
      const events = dayTasks.map((t) => ({
        ...
        description:
          [t.affaire_num ? `Affaire ${t.affaire_num}` : '', t.notes || '']
            .filter(Boolean)
            .join(' — ') || '',
  ...
  app.post(
    ...
    (req, res) => {
      try {
        ...
        const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
        db.prepare(
          'DELETE FROM display_completed_events WHERE event_id = ? AND event_date = ?',
        ).run(String(eventId), dateStr);
        res.json({ success: true, eventId });
      } catch (error) {
