...
const ReservationModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    locationName:
      reservation?.locationName || googleEvent?.detectedLocation || googleEvent?.location || '',
  ...
  const { suggestions: _locationSuggestions, addToHistory: addLocation } =
    useAutocomplete('locations');
  const { suggestions: prestationSuggestions, addToHistory: addPrestation } =
    useAutocomplete('prestations');
  const { suggestions: _affaireSuggestions, addToHistory: addAffaire } =
    useAutocomplete('affaires');
  ...
  const qualifiedDrivers = React.useMemo(() => {
    ...
    if (
      ['PL', 'CAMION', 'PORTEUR', 'PORTEUR MOYEN', 'TRACTEUR'].some((t) => vehicleType.includes(t))
    ) {
  ...
  const [selectedVehicleIds, setSelectedVehicleIds] = useState(isMultiVehicle ? [] : []);
  ...
  const handleOpenCombinedTripDetails = (groupEventItems) => {
    setSelectedEventsForCombinedTrip(
      groupEventItems.map((item, _index) => ({
        event: item.event,
          ...
          <fieldset disabled={isReadOnly} className="reservation-fieldset">
            ...
            <div className="form-section">
              ...
              {isMultiVehicle ? (
                <FormField
                  ...
                  label="Véhicules * (Sélectionnez un ou plusieurs véhicules)"
                >
                  <VehiclePickerCards
                    vehicles={vehicles}
                ...
                <>
                  <FormField className="form-group" label="Véhicule" htmlFor="vehicleId" required>
                    <VehiclePickerCards
                      ...
                      onSelect={(id) =>
                        !isReadOnly && handleChange({ target: { name: 'vehicleId', value: id } })
                      }
                      disabled={isReadOnly}
                      variant="desktop"
                    />
                            ...
                            >
                              {filteredLocations.map((location) => (
                                ...
                                >
                                  ...
                                  {location.address && (
                                    <div className="reservation-location-item-address">
                                      {location.address}
                                    </div>
                                  )}
                                  {location.type && (
                                    <div className="reservation-location-item-type">
                                      {location.type}
                                    </div>
                                  )}
                                </div>
            ...
            <div className="form-section">
              ...
              {!isMultiVehicle && googleEvents.length > 0 && (
                <FormField
                  className="form-group"
                  label="Lier à un événement Google (optionnel)"
                  htmlFor="googleEventSelect"
                >
                          ...
                          <div className="reservation-tournee-events">
                            {formData.linkedEventIds.map((eventId) => {
                              ...
                              const dateRange = startDate
                                ? format(startDate, 'dd/MM', { locale: fr })
                                : '';
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {event.affaire && (
                                    <span className="event-affaire reservation-event-date-xs">
                                      {event.affaire}
                                    </span>
                                  )}
                                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                              ...
                              <span className="reservation-event-affaire-badge">
                                {affairesWithAttachments.includes(event.affaire) && (
                                  <Paperclip
                                    ...
                                    title={`${attachmentCounts[event.affaire] || ''} pièce(s) jointe(s)`}
                                  />
                                )}
                                {affairesWithAttachments.includes(event.affaire) &&
                                  attachmentCounts[event.affaire] && (
                                    <span className="u-opacity-70" style={{ fontSize: '0.6rem' }}>
                                      {attachmentCounts[event.affaire]}
                                    </span>
                                  )}
                                {event.affaire}
                          ...
                          <div className="reservation-event-title">{cleanTitle}</div>
                          {event.location && (
                            <div className="reservation-event-location">📍 {event.location}</div>
                          )}
                        </div>
                        ...
                        {!isInGroup && isEdit && (
                          <div className="trip-link-actions">
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setLinkEventComboboxOpen(
                                  linkEventComboboxOpen === eventId ? null : eventId,
                                );
                              }}
                            ...
                            {linkEventComboboxOpen === eventId && (
                              ...
                              >
                                ...
                                <div className="combobox-list">
                                  ...
                                  {formData.linkedEventIds
                                    ...
                                    .map((ge) => {
                                      ...
                                      const geStart = ge.start?.dateTime
                                        ? new Date(ge.start.dateTime)
                                        : ge.start?.date
                                          ? new Date(ge.start.date)
                                          : null;

                                      return (
                                        <div
                                          key={ge.id}
                                          className="combobox-item combobox-item-tournee"
                                          onClick={() => {
                                        ...
                                        >
                                          <span className="combobox-date">
                                            🚐{' '}
                                            {geStart
                                              ? format(geStart, 'dd/MM', { locale: fr })
                                              : ''}
                                          </span>
                                          <span className="combobox-title">{geTitle}</span>
                                          {ge.affaire && (
                                            <span className="combobox-affaire">{ge.affaire}</span>
                                          )}
                                        </div>
                                      );
                                    })}
                                  {/* Séparateur si les deux listes ont des éléments */}
                                  {formData.linkedEventIds.filter((eid) => eid !== eventId).length >
                                    0 &&
                                    googleEvents.filter(
                                      (ge) =>
                                        ge.id !== eventId &&
                                        !formData.linkedEventIds.includes(ge.id),
                                    ).length > 0 && (
                                ...
                                <Button
                                  ...
                                  onClick={() => setLinkEventComboboxOpen(null)}
                                >
                                  Fermer
                                </Button>
                              </div>
                      ...
                      <div key={key} className="trip-link-separator">
                        {areLinked ? (
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleLinkTrips(lastEventId, firstEventId);
                            }}
                      ...
                      <div className="reservation-events-stack">
                        {segments.map((segment, segIdx) => {
                          ...
                          if (segIdx > 0) {
                            ...
                            const prevLastEventId =
                              prevSegment.items[prevSegment.items.length - 1].eventId;
                                      ...
                                      {itemIdx > 0 && (
                                        <div className="group-inner-separator">
                                          ...
                                          <Button
                                            variant="ghost"
                                            type="button"
                                            className="unlink-inner-btn"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleUnlinkTrip(item.eventId);
                                            }}
            ...
            {isEdit && reservation?.id && (
              <>
                <Suspense
                  fallback={
                    <div className="reservation-loading-fallback">Chargement matériel...</div>
                  }
                >
                  <ReservationEquipment reservationId={reservation.id} currentUser={currentUser} />
                </Suspense>
              </>
            )}
          </fieldset>
        ...
        (() => {
          ...
          return (
            <TripDetailsModal
              ...
              vehicle={selectedVehicle}
              nextEvent={
                selectedEventsForCombinedTrip.length > 1
                  ? selectedEventsForCombinedTrip[1].event
                  : null
              }
              googleMapsApiKey={googleMapsApiKey}
              companyAddress={companyAddress}
